...
def slowDownFile(f):
    ...
    if rating >= settings.min_rating:
        if settings.just_copy in f_name:
            shutil.copy(f_path, settings.output_path)
            print("Copied: ", f_name)
        elif settings.overwrite or not f_name[:-4] + settings.append + settings.filetype in existing_file_names:
...
if __name__ == "__main__":
    threads = []
    d = os.scandir(settings.input_path)
    try:
        os.mkdir(settings.output_path)
    except:
        pass
    existing_file_names = [n.name for n in os.scandir(settings.output_path)]
    for f in d:
